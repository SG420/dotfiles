...
# start a terminal
bindsym $mod+Return exec urxvt -fn Fixed

# kill focused window
bindsym $mod+Shift+q kill

# start program launcher
bindsym $mod+x exec --no-startup-id dmenu_recency
...
bindsym $mod+ctrl+F3 exec dolphin
...
exec --no-startup-id xrandr --output DP-0 --right-of DVI-D-0 --auto
...
#bindsym $mod+shift+0 exec pkill compton
#bindsym $mod+shift+9 exec compton
# exec --no-startup-id wal -R
exec --no-startup-id compton
exec --no-startup-id megasync
bindsym $mod+F4 exec lutris
exec --no-startup-id ckb
exec --no-startup-id ~/Desktop/tearing.sh
exec --no-startup-id ~/.res.sh
exec --no-startup-id nitrogen --restore
